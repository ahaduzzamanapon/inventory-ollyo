...
{
    $query .= 'WHERE user_id = "'.$_SESSION["user_id"].'" AND ';
}
...
if(isset($_POST["order"]))
{
    if ($_POST['order']['0']['column'] == 0) {
        $query .= 'inventory_order_id'.' '.$_POST['order']['0']['dir'].' ';
    }else{
...
while($row = $result->fetch_assoc())
{
    ...
    $sub_array[] = $status;
    $sub_array[] = date('d-F-Y',strtotime($row['inventory_order_date']));
